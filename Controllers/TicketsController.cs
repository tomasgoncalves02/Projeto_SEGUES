    ...
    public class TicketsController : Controller
    {
        ...
        public async Task<IActionResult> HistoricoSenhas(string searchString, TicketState? stateFilter, string flowFilter)
        {
            ...
            var query = _context.Tickets
                ...
                .Where(t => t.OwnerId == user.Id || t.Transfers.Any(tr => tr.SenderId == user.Id || tr.ReceiverId == user.Id))
                .AsQueryable();
            ...
            if (!string.IsNullOrEmpty(flowFilter))
            {
                ...
                {
                    case "Compradas":
                        query = query.Where(t => t.OwnerId == user.Id && !t.Transfers.Any(tr => tr.ReceiverId == user.Id));
                        break;
                    case "Enviadas":
                        query = query.Where(t => t.Transfers.Any(tr => tr.SenderId == user.Id));
                        break;
                    case "Recebidas":
                        query = query.Where(t => t.Transfers.Any(tr => tr.ReceiverId == user.Id));
                        break;
                }
            }
            ...
            var tickets = await query.OrderByDescending(t => t.PurchaseDate).ToListAsync();
            return View(tickets);
        }
