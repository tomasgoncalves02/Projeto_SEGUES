...
public class TicketIntegrationTests
{
    ...
    private static AppUser CreatePedro(string id, string email, UserCategory cat) => new()
    {
        Id = id,
        FirstName = "Pedro",
    ...
    public async Task TransferTicketsAsync_AtomicOperation_ChangesOwnerAndLogsTransfer()
    {
        ...
        await service.TransferTicketsAsync("s1", "receiver@test.pt", ["TRANSFER-1"]);

        var dbTicket = await context.Ticket.Include(t => t.Owner).FirstAsync();
        var transferRecord = await context.TicketTransfer.Include(ticketTransfer => ticketTransfer.Sender)
            .Include(ticketTransfer => ticketTransfer.Receiver).FirstOrDefaultAsync(tr => tr.Ticket.ValidationCode == "TRANSFER-1");
