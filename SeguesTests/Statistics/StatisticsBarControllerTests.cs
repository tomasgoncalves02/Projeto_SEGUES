...
{
    public class StatisticsBarControllerTests
    {
        private readonly Mock<IStatisticsService> _mockStatisticsService;
        private readonly StatisticsBarController _controller;

        public StatisticsBarControllerTests()
        {
            _mockStatisticsService = new Mock<IStatisticsService>();
            _controller = new StatisticsBarController(_mockStatisticsService.Object);
        }
        ...
        public async Task GetBarStats_ValidPeriod_ReturnsJsonResult()
        {
            ...
            _mockStatisticsService.Setup(s => s.GetBarStats(1)).ReturnsAsync(testData);

            var result = await _controller.GetBarStats(1);
        ...
        {
            int period = 7;
            await _controller.GetBarStats(period);

            _mockStatisticsService.Verify(s => s.GetBarStats(period), Times.Once);
        }
        ...
        {
            _mockStatisticsService.Setup(s => s.GetBarStats(It.IsAny<int>())).ReturnsAsync((object)null!);
